...
from scripts.setup_minio import (
    BRONZE_BUCKET_NAME,
    SILVER_BUCKET_NAME,
    MINIO_ENDPOINT,
    MINIO_ACCESS_KEY,
...
spark = (
    ...
    .config(
        "spark.jars.packages",
        f"io.delta:delta-spark_2.12:{DELTA_VERSION},org.apache.hadoop:hadoop-aws:{HADOOP_VERSION}",
    )
...
def extract_data_with_duplicates(
    bucket_name: str, topic: str, dedup_columns: list
) -> DataFrame:
...
def main():
    ...
    playbacks_df = extract_data_with_duplicates(
        BRONZE_BUCKET_NAME, "raw_playbacks", ["user_id", "movie_id", "click_ts"]
    )
    ...
    payments_df = extract_data_without_duplicates(
        BRONZE_BUCKET_NAME, "raw_payment_attempts"
    )
