...
def process_scd2_dimension(
    ...
    Contains a temporary testing patch to set valid_from_ts to 180 days before 2026-06-07.
    """
    ...
    if not DeltaTable.isDeltaTable(spark, target_path):
        ...
        # --- TESTING PATCH START ---
        # Dynamically subtracting 180 days from the 2026-06-07 anchor date
        print(f"  [PATCH] Forcing historical timeline back 180 days from 2026-06-07 for {topic} bootstrap.")
        anchor_date = F.lit("2026-06-07")
        dynamic_start_date = F.date_sub(anchor_date, 180)
        
        initial_df = silver_df.select([bk_col] + attribute_cols + [valid_from_col, "_ingested_at"]) \
            .withColumn(sk_col, F.md5(F.concat(F.col(bk_col).cast("string"), F.col("_ingested_at").cast("string")))) \
            .withColumn("valid_from_ts", F.to_timestamp(dynamic_start_date)) \
    ...
    target_table.alias("tgt").merge(
        ...
        set={
            "valid_to_ts": f"src.{valid_from_col}",  
            "is_current": "false"
        }
        ...
        values={
            ...
            **{c: f"src.{c}" for c in attribute_cols},
            "valid_from_ts": f"src.{valid_from_col}",
...
if __name__ == "__main__":
    ...
    # Process dim_user -> Anchored on user signup timeline
    ...
    process_scd2_dimension(
        ...
        valid_from_col="signup_ts"
    )

    # Process dim_movie -> Anchored on original metadata creation timestamp
